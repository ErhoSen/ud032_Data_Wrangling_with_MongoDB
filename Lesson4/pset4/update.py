...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            label = line.get('rdf-schema#label')
            binomial = line.get('binomialAuthority_label')
            if binomial != "NULL":
                label = normolize_label(label)
                data[label] = binomial
...
def update_db(data, db):
    for label, binimial in data.items():
        item = db.arachnid.find_one({"label":label})
        item["classification"]["binomialAuthority"] = binimial
        db.arachnid.save(item)

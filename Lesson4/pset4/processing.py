...
def is_not_alphanumeric(s):
    return re.match('^[\w-]+$', s) is None

def normolize_label(label):
    if label == "NULL":
        return None
    else:
        result = ""
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            elem = {}
            classification = {"family" : None, "class": None, "phylum": None, "order": None, "kingdom": None, "genus": None}
            for key, value in FIELDS.items():
                item = line.get(key, False)
                if item:
                    #print key, item
                    if value == 'name' and (item == "NULL" or is_not_alphanumeric(item)):
                        elem[value] = elem.get('label')
                    elif value == 'label':
                        elem[value] = normolize_label(item)
                    elif value in classification:
                        classification[value] = item if item != "NULL" else None
                    elif item == "NULL":
                        elem[key] = None
                    elif value == 'synonym':
                        elem[value] = parse_array(item)
                    else:
                        elem[value] = item.strip(" ")
                else:
                    #print key, item
                    pass
            elem['classification'] = classification
            data.append(elem)

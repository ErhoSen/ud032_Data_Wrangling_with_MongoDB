...
def check_valid(value):
    return not (problemchars.search(value) and True)

#print check_valid("12312")

if check_valid("olol,o"):
    print "lo"

def make_address(child):
    adr = {}
    for key in child:
        #print key, child[key]
        if key == "k":
...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        address = {}
        if childrens:
            #print childrens[0].items()
            for elem in childrens:
                child = elem.attrib
                for key in child:
                    if key == "ref":
                        node_refs.append(child[key])
                    elif key == "k" and child[key][0:5] == "addr:":
                        address.update(make_address(child))
            if node_refs:
